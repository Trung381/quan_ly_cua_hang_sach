...
{
    public partial class FrmQLTL : Form
    {
        ...
        private void dGVQLTL_CellClick(object sender, DataGridViewCellEventArgs e)
        {
        ...
        private bool checkData_Control_ks()
        {
            try
            {
                ...
                //    return false;
                //}
                if (tbNameTL.Text.Trim() == "")
                {
                    MessageBox.Show("Bạn chưa nhập đủ thông tin !!!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                else if (tbMaTL.Text.Trim().Length > 5)
                {
                    MessageBox.Show("Mã NXB chỉ bao gồm 5 kí tự!!!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
                else if (tbNameTL.Text.Trim().Length > 50)
                {
        ...
        private void bt_add_category_Click(object sender, EventArgs e)
        {
            if (checkData_Control_ks())
            {
                ...
                string sqlquery_command = "select* from Category where (Ma_Loai = '" + tbMaTL.Text + "')";
                ...
                if (check_for_duplicate_information(sqlquery_command) == false)     //nếu mã sách chưa có thì thực hiện thêm dữ liệu sách mới vào CSDL
                {
                    //thêm dữ liệu
                    conn.Open();
                    string data_insert = "insert into Category values('" + tbNameTL.Text.Trim() + "')";
